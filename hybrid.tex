...
\For{$\{x,y,z\} \in V$}{
  \For{$\{u,v\} \in V\setminus\{x,y,z\}$}{
    $a_{u,v} := 
    \frac{(d_{uz}+d_{vy})-(d_{vz}+d_{uy})}{(d_{ux}+d_{vy})-(d_{vz}+d_{uy})}$\;
  }
  \If{$\forall a_{u,v}: a_{u,v}=a\in [0,1]$}{
    \eIf{$a \neq 0,1$}{
      $\delta_z \leftarrow \frac{1}{2}(d_{xz}+d_{yz}-d_{xy})$\;
      $d_{xy} \leftarrow d_{uz} - a d_{ux} - (1-a)d_{uy}-2\delta_z + 
      a d_{xz} + (1-a) d_{yz}$\;
      $\delta_x \leftarrow d_{xz} - (1-a) d_{xy} - \delta_z$\;
      $\delta_y \leftarrow d_{yz} - a \delta_z$\;
    }{
